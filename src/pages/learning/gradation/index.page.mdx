...
したがって`gl_FragCoord.x / 100.0`は0 ~ 8であり、`fract(gl_FragCoord.x / 100.0)`はx軸に基づいて、0以上1未満の値が100pxごとに、最大8回繰り返されることとなる。この値を`red`として`gl_FragColor`に格納しているので、グラデーションが繰り返される模様なった。
