...
フラグメントシェーダーはピクセルの色情報を扱うシェーダーであり、`gl_FragColor` というこちらもGLSLの組み込み変数に値を格納する。描画対象にあるピクセルのRGBA（赤、緑、青、透過度）を定義し、各ピクセルに対し、1回ずつ実行される。
...
ピクセルごとのUV座標がわかるのであれば、あとは線形補間でグラデーションすれば良いことになる。線形補間には`mix()`関数が使えるので、以下のような実装となる。
